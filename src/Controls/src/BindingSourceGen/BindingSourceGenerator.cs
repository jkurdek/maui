...
public class BindingSourceGenerator : IIncrementalGenerator
{
	...
	private static EquatableArray<DiagnosticInfo> VerifyCorrectOverload(InvocationExpressionSyntax invocation, InterceptedMethodType methodType, GeneratorSyntaxContext context, CancellationToken t) =>
		methodType switch
		{
	...
	private static EquatableArray<DiagnosticInfo> VerifyCorrectOverloadBindingFactoryCreate(InvocationExpressionSyntax invocation, GeneratorSyntaxContext context, CancellationToken t){
		...
		if (firstArgument is IdentifierNameSyntax)
		{
			...
			{
				return new EquatableArray<DiagnosticInfo>([DiagnosticsFactory.GetterIsNotLambda(firstArgument.GetLocation())]);
			}
			else // String and Binding
			{
				return new EquatableArray<DiagnosticInfo>([DiagnosticsFactory.SuboptimalSetBindingOverload(firstArgument.GetLocation())]);
			}
		}
	...
	private static EquatableArray<DiagnosticInfo> VerifyCorrectOverloadSetBinding(InvocationExpressionSyntax invocation, GeneratorSyntaxContext context, CancellationToken t)
	{
		var argumentList = invocation.ArgumentList.Arguments;
